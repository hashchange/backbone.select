 ...
 *   Copyright
 *   (c) 2014 Box, Inc.
 *   (c) 2015-2017 Michael Heim, Zeilenwechsel.de
 *
...
/**
 * A data provider for use with Mocha or Jasmine. Generates describe() blocks. Use this around a call to it() to run the
 * test over a series of data.
 *
...
/**
 * A data provider for use with Mocha or Jasmine. Generates it() blocks. Use this around test code which would normally
 * be placed into it(). Each key in the data set becomes the name of a test.
 *
 * @throws {Error}                      if dataset is missing or an empty array
 * @param  {Object|Array} dataset       the data to test
 * @param  {Function}     testFunction  the function to call for each piece of data
...
/**
 * A data provider for use with Mocha or Jasmine. Use this to run tests over a series of data.
 *
 * @throws {Error}                       if dataset is missing or an empty array
 * @param  {Function}     blockFunction  the Mocha/Jasmine function to use for generating the block, ie `describe` or `it`
 * @param  {Object|Array} dataset        the data to test
 * @param  {Function}     testFunction   the function to call for each piece of data
 ...
 */
function _withData ( blockFunction, dataset, testFunction, prefix, suffix ) {
    ...
    for ( var name in namedDataset ) {
        if ( namedDataset.hasOwnProperty( name ) ) {
            ...
            blockFunction( prefix + name + suffix, (function ( name ) {
                return function () {

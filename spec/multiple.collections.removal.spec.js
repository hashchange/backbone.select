describe( "Models shared between multiple collections: removing models with remove() and destroy()", function () {
    ...
    describe( 'The event @bbs:remove:silent, when triggered by remove()', function () {
        ...
        describe( 'in a Select.One collection', function () {
            ...
            it( 'passes along an options object as third argument, containing the previous selection state and the index of the removed models, when a series of models are removed in order', function () {
            ...
            it( 'passes along an options object as third argument, containing the previous selection state and the index of the removed models, when a series of models are removed in reverse order', function () {
                var removedModel2 = new Model(),
                    removedModel3 = new Model();
                ...
                expect( collection.trigger ).toHaveBeenCalledWithInitial( "@bbs:remove:silent", removedModel3, collection, { silent: true, index: 3, "@bbs:wasSelected": { selected: false } } );
                expect( collection.trigger ).toHaveBeenCalledWithInitial( "@bbs:remove:silent", removedModel2, collection, { silent: true, index: 2, "@bbs:wasSelected": { selected: false } } );
                expect( collection.trigger ).toHaveBeenCalledWithInitial( "@bbs:remove:silent", removedModel, collection, { silent: true, index: 0, "@bbs:wasSelected": { selected: true } } );
            } );
